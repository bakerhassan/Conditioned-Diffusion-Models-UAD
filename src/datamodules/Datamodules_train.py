    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not hasattr(self, 'train'):
            ...
            if self.cfg.sample_set:  # for debugging
                train, val, test = train[:10], val[:10], set[:10]
            self.train = get_vol2slice_dataset(train)
            self.val = get_vol2slice_dataset(val)
            self.val_eval = get_vol2slice_dataset(val)
            self.test_eval = get_vol2slice_dataset(test)
